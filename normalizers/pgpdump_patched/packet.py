...
from cryptography.hazmat.primitives.asymmetric.ec import EllipticCurvePublicNumbers, EllipticCurve, _CURVE_TYPES
...
class PublicKeyPacket(Packet, AlgoLookup):
    ...
    def parse_key_material(self, offset):
        if self.raw_pub_algorithm in (1, 2, 3):
            ...
            self.key_size = int(ceil(log(self.prime, 2)))
            ...
            pass
        elif self.raw_pub_algorithm in (22,):
            # EdDSA
            self.pub_algorithm_type = "eddsa"
            ...
            raw_mpi_data = self.data[offset:]
            ec_point, offset = get_mpi(self.data, offset)

            if raw_mpi_data[2] != 0x40:
                raise Exception("Invalid mpi byte 0x40, actual value: {}".format(raw_mpi_data[2]))

            point_data = raw_mpi_data[3:]
            x, y, _, _ = ed25519.decodepoint(point_data)
            ...
            self.key_size = 256
        elif self.raw_pub_algorithm in (18,):
            ...
            ec_point, offset = get_mpi(self.data, offset)
            raw_mpi_data = self.data[offset_before_point:offset]
            x, y, key_size = self.decode_ec_point(self.curve, ec_point, raw_mpi_data, "ecdh")
            self.x = x
            self.y = y
            self.key_size = key_size

            # KDF parameters
            kdf_size = self.data[offset]
            offset += 1
            kdf_01 = self.data[offset]
            offset += 1
            if kdf_01 != 1:
                raise Exception("Invalid ECDH key format. KDF reserved byte is not equal to 1. It equals: {}".format(kdf_01))
            hash_function_id = self.data[offset]
            offset += 1
            symmetric_algo_id = self.data[offset]
            offset += 1

            kdf_variable_length_data = self.data[offset:offset+20]
            # offset += kdf_size
            remaining_bytes = kdf_size - 3
            offset += remaining_bytes
        elif self.raw_pub_algorithm in (19,):
            # ECDSA
            """
            RFC 6637, section 9
            ...
            ec_point, offset = get_mpi(self.data, offset)
            # decode point
            x, y, key_size = self.decode_ec_point(self.curve, ec_point, raw_mpi_data, "ecdsa")
            self.x = x
            self.y = y
            self.key_size = key_size
    ...
    def decode_ec_point(self, curve, ec_point_mpi, raw_mpi_data, source):
        if curve == "Curve25519":
            point_data = raw_mpi_data[3:]
            x = None
            y = None
    ...
    def curve_name_for_oid(self, oid):
        string_oid = ", ".join(hex(b) for b in oid)
        oid = list(oid)
        if oid == [0x2a, 0x86, 0x48, 0xce, 0x3d, 0x03, 0x01, 0x07]:
            return "secp256k1"
        elif oid == [0x2b, 0x81, 0x4, 0x0, 0xa]:
            return "secp256k1"
        elif oid == [0x2b, 0x81, 0x4, 0x0, 0x22]:
            return "secp384r1"
        elif oid == [0x2b, 0x81, 0x4, 0x0, 0x23]:
            return "secp521r1"
        # Brainpool RFC: https://tools.ietf.org/html/rfc5639#section-4.1
        elif oid == [0x2b, 0x24, 0x3, 0x3, 0x2, 0x8, 0x1, 0x1, 0x7]:
            return "brainpoolP256r1"
        elif oid == [0x2b, 0x24, 0x3, 0x3, 0x2, 0x8, 0x1, 0x1, 0xb]:
            return "brainpoolP384r1"
        elif oid == [0x2b, 0x24, 0x3, 0x3, 0x2, 0x8, 0x1, 0x1, 0xd]:
            return "brainpoolP512r1"
        elif oid == [0x2b, 0x6, 0x1, 0x4, 0x1, 0x97, 0x55, 0x1, 0x5, 0x1]:
            return "Curve25519"
        else:
            return string_oid
